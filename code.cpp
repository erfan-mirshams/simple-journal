...
public:
  ...
  bool operator<=(const date &b) {
    return make_pair(make_pair(year, month), day) <=
           make_pair(make_pair(b.year, b.month), b.day);
  }
...
vector<diary>::iterator find_diary_with_highest_val(vector<diary>::iterator st,
                                                    vector<diary>::iterator fn,
                                                    int (diary::*get_val)()) {
...
vector<diary>::iterator find_diary_from_date_string(vector<diary>::iterator st,
                                                    vector<diary>::iterator fn,
                                                    string date_str) {
  return find_if(st, fn,
                 [&](diary cur) { return cur.get_date() == date(date_str); });
}
...
int main() {
  ...
  string commands[COMMANDSIZE] = {"start_day", "show_day",
                                  "show_the_longest_day", "show_the_best_day"};
  ...
  while (cin.peek() != EOF) {
    ...
    if (i == COMMANDSIZE) {
      ...
      if (date_str_temp.compare("")) {
        vector<diary>::iterator diary_it = find_diary_from_date_string(
            diaries.begin(), diaries.end(), date_str_temp);
    ...
    case 1: {
      vector<diary>::iterator diary_it = find_diary_from_date_string(
          diaries.begin(), diaries.end(), word_from_position(it, line.end()));
      diary_it->print_full();
    } break;
    case 2: {
      vector<diary>::iterator diary_it = find_diary_with_highest_val(
          diaries.begin(), diaries.end(), &diary::get_content_size);
      diary_it->print_summary();
    } break;
    case 3: {
      vector<diary>::iterator diary_it = find_diary_with_highest_val(
          diaries.begin(), diaries.end(), &diary::get_pos_count);
      diary_it->print_summary();
    } break;
    }
  }
